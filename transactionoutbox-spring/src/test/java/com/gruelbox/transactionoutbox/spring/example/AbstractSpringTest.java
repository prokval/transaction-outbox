...
public abstract class AbstractSpringTest {
  ...
  @Autowired private ExternalQueueServiceResultHandler externalQueueServiceResultHandler;
  @Autowired private NonOutboxExternalQueueService nonOutboxExternalQueueService;
  ...
  void test_01_CheckCustomerCreation() throws Exception {
    ...
    await()
        ...
        .untilAsserted(
            () ->
                assertThat(externalQueueServiceResultHandler.getRecords())
                    .containsExactlyInAnyOrder(
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            joe, null, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            dave, null, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            neil, null, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            tupac, null, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            jeff, null, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            joe, null, joe.getId(), null),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            dave, null, dave.getId(), null),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            neil, null, neil.getId(), null),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            tupac, null, tupac.getId(), null),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            jeff, null, jeff.getId(), null)));
  }
  ...
  void test_02_CheckAddPayments() {
    ...
    assertTrue(template.postForEntity(url, joe, Void.class, 25).getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, tupac, Void.class, 55).getStatusCode().is2xxSuccessful());
    ...
    assertTrue(
        template.postForEntity(url, tupac, Void.class, 10).getStatusCode().is2xxSuccessful());

    Map<Long, List<Integer>> expected =
        Map.of(
    ...
    await()
        ...
        .pollDelay(1, SECONDS)
        .untilAsserted(
            () -> assertThat(externalQueueService.getPayments()).containsAllEntriesOf(expected));
        ...
        .untilAsserted(
            () ->
                assertThat(externalQueueServiceResultHandler.getRecords())
                    .containsExactlyInAnyOrder(
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            joe, 30, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            dave, 11, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            neil, 10, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            tupac, 55, null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            jeff, 1, null, "Temporary failure, try again"),
  ...
  void test_03_CheckUpdateStatus() {
    ...
    assertTrue(
        template.postForEntity(url, joe, Void.class, "ST1").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, dave, Void.class, "ST1").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, joe, Void.class, "ST2").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, dave, Void.class, "ST2").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, joe, Void.class, "ST3").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, neil, Void.class, "FIN").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, joe, Void.class, "FIN").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, tupac, Void.class, "ST1").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, dave, Void.class, "FIN").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, jeff, Void.class, "FIN").getStatusCode().is2xxSuccessful());
    assertTrue(
        template.postForEntity(url, tupac, Void.class, "FIN").getStatusCode().is2xxSuccessful());
    ...
    await()
        ...
        .pollDelay(1, SECONDS)
        .untilAsserted(
            () -> assertThat(externalQueueService.getStatuses()).containsAllEntriesOf(expected));
        ...
        .untilAsserted(
            () ->
                assertThat(externalQueueServiceResultHandler.getRecords())
                    .containsExactlyInAnyOrder(
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            joe, "FIN", null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            dave, "FIN", null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            neil, "FIN", null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            tupac, "FIN", null, "Temporary failure, try again"),
                        new ExternalQueueServiceResultHandler.ResultRecord(
                            jeff, "FIN", null, "Temporary failure, try again"),
  ...
  @Test
  void test_04_NonOutboxService_works_without_TransactionalOutbox_processing() {
