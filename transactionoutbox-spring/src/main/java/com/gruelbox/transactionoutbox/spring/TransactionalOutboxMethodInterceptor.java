...
class TransactionalOutboxMethodInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        if (annotation != null) {
            Duration delayForAtLeast = annotation.delayMillis() > 0 ?
                    Duration.ofMillis(annotation.delayMillis()) : null;

            String topic = generateTopic(annotation.useBeanMethodAsTopic(), annotation.orderExpression(), invocation);

            transactionOutbox.with()
                    .ordered(topic)
                    .orderedTakeLast(annotation.orderedTakeLast())
                    .delayForAtLeast(delayForAtLeast)
                    .persistInvocationAndAddPostCommitHook(invocation.getMethod(), invocation.getArguments(), annotation.requireInTransaction());
    ...
    private String generateTopic(boolean useBeanMethodAsTopic, String topicExpression, MethodInvocation invocation) {
        ...
        if (!ObjectUtils.isEmpty(topicExpression)) {
            ...
            EvaluationContext context = new MethodBasedEvaluationContext(null, invocation.getMethod(), invocation.getArguments(), parameterNameDiscoverer);
