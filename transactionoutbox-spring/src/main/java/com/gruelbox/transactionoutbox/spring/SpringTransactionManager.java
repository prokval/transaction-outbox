...
public class SpringTransactionManager implements ThreadLocalContextTransactionManager {
    ...
    @Autowired
    public SpringTransactionManager(DataSource dataSource, PlatformTransactionManager transactionManager) {
    ...
    private static TransactionTemplate createTransactionTemplate(PlatformTransactionManager transactionManager, Propagation propagation) {
    ...
    public <T, E extends Exception> T inTransactionReturnsThrows(ThrowingTransactionalSupplier<T, E> work) throws E {
        try {
            return requiresNewTxTemplate.execute(t -> uncheckedly(() -> work.doWork(transactionInstance)));
