...
@Component
@Slf4j
public class TransactionalOutboxMethodRegistry implements TransactionOutboxListener, ApplicationContextAware,
    ...
    private void callResultHandler(TransactionOutboxEntry entry, Object result, Throwable cause) {
        ...
        BeanMethodKey key = new BeanMethodKey(invocation.getClassName(), invocation.getMethodName(),
    ...
    private static final class ResultHandlerCaller implements MethodDescriptor {
        ...
        void call(Object[] args, Object result, Throwable error) {
            Object[] methodArgs = noResultParameter ?
    ...
    private static final class TransactionalOutboxMethod implements MethodDescriptor {
        ...
        MethodKey getRequredResultHandlerMethodKey() {
            Class<?>[] resultHandlerMethodParameterTypes = isVoidReturnType() ?
                    ArrayUtils.addAll(method.getParameterTypes(), Throwable.class) :
                    ArrayUtils.addAll(method.getParameterTypes(), method.getReturnType(), Throwable.class);
    ...
    void registerTransactionalOutboxMethod(String beanName, Method method, TransactionalOutbox annotation) {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        resultHandlerCallers = transactionOutboxMethods.values().stream()
                .filter(m -> m.getAnnotation().requireResultHandler())
                .collect(Collectors.toMap(TransactionalOutboxMethod::getKey, this::buildResultHandlerMethodCallerOrThrow));
    }
    ...
    private ResultHandlerCaller buildResultHandlerMethodCallerOrThrow(TransactionalOutboxMethod transactionalOutboxMethod) {

        ResultHandlerMethod resultHandlerMethod = findResultHandlerMethodOrThrow(transactionalOutboxMethod);
        Object bean = applicationContext.getBean(resultHandlerMethod.getBeanName());

        return new ResultHandlerCaller(bean, resultHandlerMethod.getMethod(), transactionalOutboxMethod.isVoidReturnType());
    }

    private ResultHandlerMethod findResultHandlerMethodOrThrow(TransactionalOutboxMethod transactionalOutboxMethod) {
        MethodKey keyToFind = transactionalOutboxMethod.getRequredResultHandlerMethodKey();

        Map<MethodKey, List<ResultHandlerMethod>> handlersByKey = resultHandlerMethods.stream()
                .collect(Collectors.groupingBy(ResultHandlerMethod::getKey));

        List<ResultHandlerMethod> candidates = handlersByKey.get(keyToFind);
        if (CollectionUtils.isEmpty(candidates)) {
            throw new ApplicationContextException("Can't find @TransactionalOutboxResultHandler for " + transactionalOutboxMethod);
        }

        if (candidates.size() > 1) {
            throw new ApplicationContextException("Ambiguity in @TransactionalOutboxResultHandler among " +
                                                  candidates.stream().map(ResultHandlerMethod::toString).collect(Collectors.joining(", ")));
        }
