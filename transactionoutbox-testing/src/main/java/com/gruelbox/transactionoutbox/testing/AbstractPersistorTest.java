...
public abstract class AbstractPersistorTest {
  ...
  /**
   * For PgSeqDialect.POSTGRESQL_SEQ dialect sequence is filled by Database automatically. To meet
   * our expectations, we might need to set it to null when read records from the DB
   */
  private List<TransactionOutboxEntry> setSequenceToNull(
      Collection<TransactionOutboxEntry> records) {
  ...
  void testOrderedSelect() throws Exception {
    ...
    txManager()
        .inTransactionThrows(
            tx ->
                assertThat(
                    persistor().selectNextInTopics(tx, 100, now.plusMillis(1)),
                    containsInAnyOrder(entry11, entry23, entry33, entry51)));
  }
  ...
  private TransactionOutboxEntry createEntry(
      String id, Instant nextAttemptTime, boolean blocked, String topic, boolean orderedTakeLast) {
    return TransactionOutboxEntry.builder()
        .id(id)
        .invocation(createInvocation())
        .blocked(blocked)
